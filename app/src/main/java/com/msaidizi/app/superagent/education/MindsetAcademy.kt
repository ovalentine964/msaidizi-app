...
/**
 * Mindset Academy — bite-sized financial education for informal workers.
 *
 * Delivers 2-3 minute voice lessons on wealth mindset, financial literacy,
 * and personal development. All content in Swahili first, English fallback.
 *
 * ## Content Sources
 ...
 * - East African financial literacy fundamentals
 *
 * ## Categories
 * - **HABITS** — Daily wealth-building routines
 * - **GOALS** — Setting and achieving financial targets
 * - **FINANCIAL_LITERACY** — Profit, pricing, cash flow
 * - **MINDSET** — Thinking like an entrepreneur
 * - **GIVING** — Tithe, charity, community investment
 *
 * ## Voice Delivery
 * Each lesson is designed for voice output — short sentences, simple vocabulary,
 * cultural references familiar to Kenyan workers. No jargon.
 *
 * ## Progression
 * Lessons are ordered by complexity. Worker progresses from basic concepts
 * (what is profit?) to advanced topics (pricing strategy, cash flow management).
 * Each completed lesson earns 8 gamification points.
 *
 * @param lessonProvider Provides access to lesson storage
 */
class MindsetAcademy(
    ...
    /** Cached lesson library — lazily initialized */
    private var lessonCache: List<Lesson>? = null

    /**
     * Get the next undelivered lesson for the worker.
     * Prioritizes lessons in order, respects category filter.
     *
     * @param category Optional category filter (null = next in sequence)
     * @return Next lesson to deliver, or null if all completed
     */
    suspend fun getNextLesson(category: String? = null): Lesson? {
        val delivered = lessonProvider.getDeliveredIds()
        ...
        return candidates.firstOrNull { it.id !in delivered }
    }
    ...
    suspend fun deliverLesson(lessonId: String, language: String = "sw"): VoiceLesson? {
        val lesson = getLessons().find { it.id == lessonId } ?: return null
        lessonProvider.markDelivered(lessonId)

        Timber.d(TAG, "Delivering lesson: %s", lessonId)

        return VoiceLesson(
            lessonId = lesson.id,
            title = if (language == "sw") lesson.titleSw else lesson.titleEn,
            content = if (language == "sw") lesson.contentSw else lesson.contentEn,
    ...
    suspend fun completeLesson(lessonId: String) {
        lessonProvider.markCompleted(lessonId)
    ...
    suspend fun getProgress(): AcademyProgress {
        val completed = lessonProvider.getCompletedCount()
        val total = getLessons().size

        val categoryProgress = CATEGORIES.associateWith { cat ->
            val catCompleted = lessonProvider.getCompletedCountByCategory(cat)
            val catTotal = getLessons().count { it.category == cat }
            CategoryProgress(cat, CATEGORIES_SW[cat] ?: cat, catCompleted, catTotal)
        }
    ...
    /**
     * Get daily lesson prompt for morning briefing.
     */
    ...
    /**
     * Get lessons by category for browsing.
     */
    suspend fun getLessonsByCategory(category: String): List<Lesson> {
    ...
    private fun getLessons(): List<Lesson> {
        return lessonCache ?: buildLessonLibrary().also { lessonCache = it }
    }

    private fun buildLessonLibrary(): List<Lesson> {
        var order = 0
        return mutableListOf<Lesson>().apply {
            ...
            add(lesson(order++, "HABITS",
                ...
                "Mfano: Ukimaliza kuuza, angalia salio lako mara moja. " +
    ...
    private fun lesson(
        ...
        id = "lesson_${order.toString().padStart(3, '0')}",
        category = category,
        titleSw = titleSw,
        titleEn = titleEn,
        contentSw = contentSw,
        contentEn = contentSw, // Use Swahili as English fallback
...
/**
 * Voice-ready lesson for TTS playback.
 */
...
/**
 * Interface for lesson storage — implemented by the database layer.
 */
interface LessonProvider {
    suspend fun getDeliveredIds(): Set<String>
    suspend fun markDelivered(lessonId: String)
